<template>
  <div class="min-h-screen bg-white">
    <!-- Vision & Mission Section -->
    <section id="about" class="py-2 bg-white">
      <div class="container mx-auto px-4 max-w-6xl">
        <div class="text-center mb-20">
          <h2 class="text-5xl font-bold text-gray-900 mb-6">Our Purpose</h2>
          <p class="text-xl text-gray-600 max-w-3xl mx-auto">
            Dedicated to creating lasting change in the lives of Cambodian children and youth
          </p>
        </div>
        <div class="grid lg:grid-cols-2 gap-16 mb-20 items-center">
          <div class="space-y-8">
            <div class="border-l-4 border-gray-900 pl-8">
              <h3 class="text-3xl font-bold text-gray-900 mb-6">Our Vision</h3>
              <p class="text-2xl text-gray-700 leading-relaxed mb-8">
                Every child and youth enjoys full development potential with dignity and sustainability.
              </p>
            </div>
            <div class="border-l-4 border-gray-400 pl-8">
              <h3 class="text-3xl font-bold text-gray-900 mb-6">Our Mission</h3>
              <p class="text-2xl text-gray-700 leading-relaxed">
                To support human resource development and livelihoods improvement, enabling participation opportunities for communities, public institutions, civil society, the private sector and other relevant stakeholders.
              </p>
            </div>
          </div>
          <div class="relative flex justify-center">
            <img src="https://live.staticflickr.com/60/188731500_27a39c9eb1_b.jpg" alt="Children Learning" class="w-full h-96 object-cover rounded-2xl shadow-2xl">
            <div class="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent rounded-2xl"></div>
          </div>
        </div>
        <!-- Strategic Goal -->
        <div class="w-fullgrid lg:grid-cols-2 gap-16 mb-20 items-center">
          <div class="grid lg:grid-cols-2 gap- items-center">
            <div class="border-l-4 border-gray-300 pl-8">
              <h3 class="text-3xl font-bold text-gray-900 mb-6">Strategic Goal</h3>
              <p class="text-2xl text-gray-700 leading-relaxed ">
                "By 2019, children and youths enjoy full potential of their rights in living with dignity to become human capital for sustainable development of the society."
              </p>
            </div>
            <div class="relative flex justify-center">
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS73PV9jSPJNx-hfjw0lrEs44ZrGDoLoXhGbtY6zbP_oSCV7ddL2xM_7p6Fa9lDvTbH0f8&usqp=CAU" alt="Strategic Vision" class="w-full h-80 object-cover rounded-2xl shadow-xl">
            </div>
          </div>
        </div>
      </div>
    </section>
    <!-- Core Values Section -->
    <section id="values" class="py-8 bg-white">
      <div class="container mx-auto px-4 max-w-6xl">
        <div class="text-center mb-20">
          <h2 class="text-5xl font-bold text-gray-900 mb-6">Our Core Values</h2>
          <p class="text-xl text-gray-600 max-w-3xl mx-auto">
            The fundamental principles that guide our work and shape our impact
          </p>
        </div>
        <div class="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h3 class="text-3xl font-bold text-gray-900 mb-6">What We Stand For</h3>
            <ul class="space-y-4">
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Accountability</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Dignity</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Equity</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Creativity</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Best Interests of Children</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Gender Responsiveness</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Cooperation and Partnership</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Resilience and Flexibility</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Child Rights Programming</b></span>
              </li>
              <li class="flex items-start">
                <span class="text-green-600 mr-3 mt-1">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                </span>
                <span><b>Result-based approach</b></span>
              </li>
            </ul>
          </div>
          <div class="flex flex-col items-center">
            <img 
              src="https://live.staticflickr.com/60/188731500_27a39c9eb1_b.jpg" 
              alt="Core Values" 
              class="rounded-xl shadow-lg w-full max-w-lg md:max-w-xl lg:max-w-2xl mb-8"
            >
            <p class="text-lg text-gray-700 text-center">
              These values guide every decision we make and every action we take, ensuring that we remain committed to empowering children and communities with respect, fairness, and innovation.
            </p>
          </div>
        </div>
      </div>
    </section>
    <!-- Our Partners Section -->
    <section id="partners" class="py-24 bg-gray">
      <div>
      
          <PartnerView />

      </div>
    </section>

  </div>
</template>


<script setup>
import { ref } from 'vue'
import PartnerView from '@/views/PartnerView.vue'


const timelineContainer = ref(null)

const scrollLeft = () => {
  if (timelineContainer.value) {
    timelineContainer.value.scrollBy({
      left: -320, // Scroll by one card width
      behavior: 'smooth'
    })
  }
}

const scrollRight = () => {
  if (timelineContainer.value) {
    timelineContainer.value.scrollBy({
      left: 320, // Scroll by one card width
      behavior: 'smooth'
    })
  }
}
</script>

<style scoped>



</style>
